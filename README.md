...
Сейчас runtime поддерживает два режима хранения:

- **in-memory** — режим по умолчанию для локальной итерации и быстрых тестов;
- **Postgres** — runtime backend для workspace assets, run history, step snapshots и persisted `run_events`;
- **Redis** для асинхронной очереди;
- **Prometheus metrics** на `/metrics`;
- **OpenTelemetry tracing** с stdout-exporter по умолчанию.

Чтобы включить durable runtime storage, укажите `STAGEFLOW_STORAGE_BACKEND=postgres`, настройте `STAGEFLOW_POSTGRES_DSN` и примените миграции перед запуском приложения.
...
- страницы run details с форматированными execution snapshots, persisted live logs и auto-refresh для активных запусков.
...
| `STAGEFLOW_STORAGE_BACKEND` | backend хранения runtime (`inmemory` или `postgres`) | `inmemory` |
| `STAGEFLOW_DEFAULT_RUN_QUEUE` | очередь для асинхронных запусков | `default` |
| `STAGEFLOW_POSTGRES_DRIVER` | имя `database/sql` драйвера для Postgres backend | `postgres` |
| `STAGEFLOW_POSTGRES_DSN` | DSN для Postgres backend | `` |
...
curl http://localhost:8080/api/v1/runs/run-000000000000001/events
```
